...
#   - JSON seeds are **best-effort**: failures are logged as WARN and do NOT fail CI.
#
...
# ------------------------------
# 3.5 Optional: Wait for Postgres
# (you already see this in your logs)
# ------------------------------
wait_for_postgres() {
  local timeout="${HEALTHCHECK_TIMEOUT:-60}"
  local elapsed=0

  log INFO "Waiting for Postgres at ${DB_HOST}:${DB_PORT} (timeout: ${timeout}s)..."
  ...
  while ! nc -z "${DB_HOST}" "${DB_PORT}" >/dev/null 2>&1; do
    ...
    if [ "${elapsed}" -ge "${timeout}" ]; then
      log ERROR "Postgres did not become ready within ${timeout}s."
      return 1
    fi
...
apply_json_seed() {
  ...
  # Run helper in a BEST-EFFORT way: do NOT fail on non-zero exit.
  set +e
  node ".github/scripts/db-seed-json.mjs" \
    --engine "${DB_ENGINE}" \
    --host "${DB_HOST}" \
  ...
  if [ "${status}" -ne 0 ]; then
    log WARN "db-seed-json.mjs exited with status ${status}; skipping JSON seed: ${json_file}"
    # Do NOT fail CI here; JSON seeds are considered optional overlays.
    return 0
  fi
}
...
# ------------------------------
# Ensure DB is up before applying fixtures
wait_for_postgres
